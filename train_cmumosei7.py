"""Train or evaluate MOCAR on the seven-class CMU-MOSEI task."""
...
from dataloader import CMUMOSEISevenDataset
...
def get_CMU_loaders(data_path, batch_size=32, valid=0.1, num_workers=0, pin_memory=False):
    trainset = CMUMOSEISevenDataset(path=data_path)
    train_sampler, valid_sampler = get_train_valid_sampler(trainset, valid, 'CMUMOSEI7')
    ...
    testset = CMUMOSEISevenDataset(path=data_path, train=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--Dataset', default='CMUMOSEI7', choices=['CMUMOSEI7'], help='dataset to train and test')
    parser.add_argument('--data-path', default='data/cmumosei_multi_regression_features.pkl', help='pre-extracted feature pickle')

    parser.add_argument('--lr', type=float, default=1.825742984741367e-4, metavar='LR', help='learning rate')
    parser.add_argument('--l2', type=float, default=3.4373060824067456e-6, metavar='L2', help='L2 regularization weight')
    ...
    parser.add_argument('--save-best-dir', default='outputs/cmumosei7/checkpoints', help='directory for best checkpoint')
    parser.add_argument('--log-dir', default='outputs/cmumosei7/logs', help='directory for console logs')
    ...
    parser.add_argument('--conf-temp', type=float, default=1.5,
    ...
    parser.add_argument('--car-lambda', type=float, default=0.003,
                        help='weight for modality-wise CAR spectral regularizer')

    parser.add_argument('--pair-margin-lambda', type=float, default=0.100,
                        help='weight for hard-pair margin loss')

    parser.add_argument('--topk-conf-pairs', type=int, default=3,
    ...
    parser.add_argument('--margin-scale', type=float, default=0.05,
    ...
    parser.add_argument('--car-fused-weight', type=float, default=1.0,
    ...
    if args.Dataset == 'CMUMOSEI7':
        ...
        train_loader, valid_loader, test_loader = get_CMU_loaders(args.data_path, valid=0.0,
                                                                  batch_size=batch_size,
                                                                  num_workers=0)
    else:
